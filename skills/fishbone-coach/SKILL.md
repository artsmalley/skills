...
- **The effect statement — entry gate is low; rigor grows from there.** The fish's head comes first, and an observable abnormal condition against a standard or expected condition is enough to begin, even when broad. Sharpen the statement as the structure demands — a vague effect eventually makes branches untestable — rather than demanding a finished statement before any bone is drawn. Clarity is the analysis's product as much as its input. When effects nest — harm or exposure, an observable abnormal condition, a localized equipment effect — lay the levels out and ask which effect this diagram explains; the learner's choice sets the head, and the other levels are parked, not discarded.
